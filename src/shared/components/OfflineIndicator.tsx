...
export function OfflineIndicator() {
  ...
  const syncPendingTransactions = useCallback(() => { console.log('sync'); }, []);

  // useOfflineDetection(); // TODO: implement
      ...
      {pendingTransactions.length > 0 && (
        <div className="bg-blue-600 text-white px-4 py-3 rounded-lg shadow-lg max-w-sm">
          ...
          <div className="text-xs text-blue-100">
            ...
            {lastSyncAt && (
              <div className="mt-1">Dernière sync : {new Date(lastSyncAt).toLocaleTimeString()}</div>
            )}
          </div>
          ...
          {!isOnline && (
            <div className="mt-2 text-xs text-blue-200">Vos données sont sauvées et se synchroniseront au retour du réseau</div>
          )}
        </div>
      )}
    </div>
